---
title: dark-survivor
published: 2025-11-12
image:
tags: [サークル, ゲーム, 公開, その他]
category: Release
draft: false
series: ゲーム開発
---
# dark-survivorについて
dark-survivorとは、ヴァンサバに影響を受けた2D Retro RPG世界観のゲームです。
このゲームは、プレイヤーが主人公として、様々なステージに挑戦しながら、世界を救う冒険を遂げるものです。

# ゲーム機能
- キャラクターのステータス管理
- マップの探索と敵との戦闘
- アイテムの入手と使用
- プレイヤーの進捗状況の保存と復元
- マップの拡張と新敵の登場

# 今後
製品版version1.0.0を公開しましたら、アップデートやDLCの開発を予定しています。
アップデートは、新しい敵やアイテムの登場、難易度の調整、グラフィックの改善などを行います。
DLCは、追加のストーリーやモード、さらなる拡張内容を提供します。

# 配布
アルファバージョンを配布しています。これは開発バージョンであり、沢山のバグや機能不足があります。
もし開発の手助けをしていただける方は、こちらのリンクから開発バージョンをダウンロードし、このサイトのコメント欄などでコメントを頂けると嬉しいです！
::link-card{url="https://github.com/Sofia-Luceat-Project/release/releases/tag/dark-survivor-version-0.0.1-alpha
" title="リリース" description="楽しんじゃうぞ！"}

# 連絡先
- こちらのコメント欄
- Twitter: @sofiagros
- Discord: sofiagros
- Github: sofia-gros

# 公式サーバー
discordサーバーに参加して最新ニュースを見よう！
::link-card{url="https://discord.gg/QZ693qAA3Z" title="sofiaとLuceatのサークル" description="Neow~"}
